"""
Full-wave characterization of the 3-layer FP antenna (fpc3_build: feed + PRS + RCM):
S11, realized boresight gain vs frequency, broadside directivity vs frequency, and
E-/H-plane patterns.  The headline question: does the RCM lift broadside gain from
the single-PRS ~11 dBi toward ~15?

Run:            python fpc3_characterize.py
Replot only:    python fpc3_characterize.py --replot
openEMS output -> fpc3_char/openems.log
"""
...
sim_path = os.path.join(os.getcwd(), 'fpc3_char')
...
def solve():
    ...
    print('Solving FPC-III (RCM=%s, output -> %s) ...' % (p1.RCM_ON, log))
    t0 = time.time()
...
def main():
    ...
    if replot:
        if not os.path.exists(data_npz):
            sys.exit('no saved data - run without --replot first')
        d = dict(np.load(data_npz)); d['f_op'] = float(d['f_op'])
    else:
        d = solve()
    ...
    print('realized gain     : %.2f .. %.2f dBi (in band)' % (float(np.min(d['Gr'])), float(np.max(d['Gr']))))
    print('f_op (best match) : %.3f GHz' % (d['f_op'] / 1e9))
    print('  (single-PRS fpc2 gave ~11 dBi; target with RCM ~15)')
    print('========================================================\n')

    fig, ax = plt.subplots(2, 2, figsize=(13, 9))
    ax[0, 0].plot(d['f']/1e9, d['s11_dB']); ax[0, 0].axhline(-10, color='r', ls='--', lw=0.8)
    ax[0, 0].axvspan(BAND_LO/1e9, BAND_HI/1e9, color='g', alpha=0.10)
    ax[0, 0].set(title='|S11| (worst in-band %.2f dB)' % worst, xlabel='GHz', ylabel='|S11| (dB)', ylim=(-40, 0))
    ...
    ax[0, 1].set(title='Broadside D & realized gain', xlabel='GHz', ylabel='dBi'); ax[0, 1].legend()
    ax[1, 0].plot(d['theta'], d['Eplane'])
    ax[1, 0].set(title='E-plane @ %.3f GHz' % (d['f_op']/1e9), xlabel='Theta (deg)',
                 ylabel='Norm (dB)', ylim=(-40, 2), xlim=(-90, 90))
    ax[1, 1].plot(d['theta'], d['Hplane'])
    ax[1, 1].set(title='H-plane @ %.3f GHz' % (d['f_op']/1e9), xlabel='Theta (deg)',
                 ylabel='Norm (dB)', ylim=(-40, 2), xlim=(-90, 90))
    for a_ in ax.flat:
        a_.grid(True)
    fig.suptitle('3-layer FP antenna (feed + PRS + RCM)', y=1.0)
    fig.tight_layout()
    out = os.path.join(sim_path, 'fpc3_characterization.png')
