"""
Realized-gain optimizer for the 2x2 via-fed PCB array (pcb_array2x2_build).

Tunes 3 knobs:
    d   element spacing      -> array directivity / sidelobes / coupling
    fv  via offset           -> the match (mutual coupling shifts it vs the element)
    L   driven patch length  -> recentre the resonance under array coupling

Objective: maximize the worst-in-band REALIZED boresight gain over 3.1-3.4 GHz,
    G = D_broadside * eta_rad * (1 - |S11|^2),
so directivity, match and loss are balanced across the band in one number.

Warm-starts from pcb_array2x2_opt/optimized_params.json if present, else the array
build defaults. Each eval is a full-array FDTD + NF2FF (a few M cells) -> minutes.

Run:  python pcb_array2x2_optimize.py
Out:  pcb_array2x2_opt/optimized_params.json | optimization_log.csv | optimized_s11.png
"""
...
PARAMS = {
    'd':  (a.d,  40.0, 80.0, 0.5),     # spacing ~0.6-0.87 lambda0
    'fv': (a.fv,  1.0, 10.0, 0.1),     # via offset = match
    'L':  (a.L,  16.0, 26.0, 0.1),     # driven length = recentre
}

WARM_START_FROM = os.path.join(os.getcwd(), 'pcb_array2x2_opt', 'optimized_params.json')
MAX_EVALS = 60
...
sim_path = os.path.join(os.getcwd(), 'pcb_array2x2_opt')
...
def update_live_plot(eval_no, f, s11_dB, label, is_best):
    ...
    try:
        ...
        ax.set(title='2x2 array realized-gain search  (%s)  eval %d' % (bt, eval_no),
...
def run_optimization():
    print('===== 2x2 ARRAY: maximize worst-in-band REALIZED GAIN over %.2f-%.2f GHz ====='
...
def report():
    ...
    print('\n--- paste into pcb_array2x2_build.py / pcb_patch_build.py ---')
    print('d  = %.2f   # pcb_array2x2_build.py' % bx['d'])
    print('fv = %.2f   # pcb_patch_build.py (element match)' % bx['fv'])
    print('L  = %.2f   # pcb_patch_build.py (driven length)' % bx['L'])
    print('------------------------------------------------------------\n')

    with open(os.path.join(sim_path, 'optimized_params.json'), 'w') as fh:
        json.dump({'objective': 'max worst-in-band realized boresight gain (2x2 array)',
...
def characterize_best():
    ...
    fig.suptitle('Optimized 2x2 array (worst-in-band Gr = %.2f dBi, d=%.1f mm)'
