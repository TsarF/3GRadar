"""
Full-wave characterization of the Fabry-Perot cavity antenna (fpc_build): one FDTD
...
Realized gain = D_broadside * eta_rad * (1 - |S11|^2).  The cavity is narrowband, so
watch where the gain actually peaks vs the -10 dB S11 span - they may not coincide.

Run:            python fpc_characterize.py           # solve + plot
Replot only:    python fpc_characterize.py --replot   # reuse saved data
openEMS output goes to fpc_char/openems.log so the console stays readable.
"""
...
sim_path = os.path.join(os.getcwd(), 'fpc_char')
...
def solve():
    ...
    print('Solving FPC (output -> %s) ...' % log)
    t0 = time.time()
...
def main():
    ...
    for a_ in ax.flat:
        a_.grid(True)
    fig.suptitle('Fabry-Perot cavity antenna', y=1.0)
    fig.tight_layout()
    out = os.path.join(sim_path, 'fpc_characterization.png')
