...
(inset_slot_build), maximizing the worst-in-band REALIZED boresight gain over
3.1-3.4 GHz:  G = D_broadside * eta_rad * (1 - |S11|^2).

Same machinery as inset_slot_optimize_de.py (population DE, process-pool parallel,
per-worker openEMS logs, incremental best-save + live log, dedup cache, warm-start,
per-eval printing) - but each eval also runs the NF2FF far field, so it's a bit
heavier than the S11-only version.

Run:  python inset_slot_optimize_de_gain.py
Out:  inset_slot_gain_de_opt/optimized_params.json | optimization_log.csv | optimized_s11.png
"""
...
NGAIN            = 5                       # in-band freqs scored for realized gain
GAIN_TARGET      = None                    # optional early stop (dBi); None = full budget
GUIDE_W          = 2.0                     # resonance-centering guidance (helps when unmatched)
...
sim_path = os.path.join(os.getcwd(), 'inset_slot_gain_de_opt')
...
def eval_worker(payload):
    """One FDTD + NF2FF solve. Returns (f, s11_dB, f_g, Gr_dBi) or None."""
...
def main():
    ...
    def live_plot(gen):
        ...
        try:
            ...
            ax.set(title='DE realized-gain | gen %d | %d evals | best %.2f dBi'
    ...
    print('===== DE realized-gain (%d workers x %d threads) over %.2f-%.2f GHz ====='
